...
export function stringsToIntegers(numbers: string[]): number[] {
    const nums = numbers.map((num: string): number => ~~num);
    return nums;
}
...
export const removeDollars = (amounts: string[]): number[] => {
    const vals = amounts.map((val: string): string =>
        val.substring(0, 1) === "$" ? val.substring(1) : val
    );
    const nums = vals.map((num: string): number => ~~num);
    return nums;
};
...
export function countShortWords(words: string[]): number {
    const wrds = words.filter((word: string): boolean => word.length < 4);
    const sum = wrds.reduce((total: number, word: string) => total + 1, 0);
    return sum;
}
...
export function injectPositive(values: number[]): number[] {
    ...
    if (!values.find((value: number): boolean => value < 0)) {
        ...
        const index = vals.findIndex((value: number): boolean => value < 0); //Finds the negative number

        const temp = vals.filter(
            (val: number): boolean =>
                vals.findIndex((value: number): boolean => value === val) <
                index
        ); //Gets rid of numbers after it

        const sum = temp.reduce((total: number, val: number) => total + val, 0);
